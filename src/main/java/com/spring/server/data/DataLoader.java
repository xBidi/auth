...
public class DataLoader implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
    ...
    final List<Scope> scopesAdmin = Arrays.asList(readScope, createScope, modifyScope, deleteScope);
    final User userAdmin =
        new User(
            adminUsername, adminEmail, adminPassword, adminEmailVerified, rolesAdmin, scopesAdmin);
    ...
    final User userUser =
        new User(userUsername, userEmail, userPassword, userEmailVerified, rolesUser, scopesUser);
    this.userRepository.save(userUser);
  }

  private boolean isDatabaseEmpty() {
    if (!this.userRepository.findAll().isEmpty()) return false;
    if (!this.roleRepository.findAll().isEmpty()) return false;
      return this.scopeRepository.findAll().isEmpty();
  }
}

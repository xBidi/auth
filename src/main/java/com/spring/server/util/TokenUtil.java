...
public abstract class TokenUtil {
    ...
    public static Map<String, Object> generateJwtClaims(final String userId,
        final List<Scope> scopes, final List<Role> roles) {
        return new HashMap<>() {{
            put("user", userId);
            put("scopes", scopes);
            put("roles", roles);
    ...
    public static String generateJwt(final String userId, final Map<String, Object> claims,
        final Date issuedDate, final Date expirationDate, final byte[] key) {
        final String jwtId = UUID.randomUUID().toString();
        return Jwts.builder().setId(jwtId).setSubject(userId).setClaims(claims)
            .setIssuedAt(issuedDate).setExpiration(expirationDate)

...
@AllArgsConstructor @NoArgsConstructor @Getter @Setter @ToString public class UserInfoOutputDto {
    ...
    public UserInfoOutputDto(User user) {
        ...
        this.setRoles(user.getRoles().stream().map(role -> role.getValue()).collect(Collectors.toList()));
        this.setScopes(user.getScopes().stream().map(scope -> scope.getValue()).collect(Collectors.toList()));
        this.setSessions(user.getSessionTokens().stream().map(sessionToken -> new TokenDto(sessionToken)).collect(Collectors.toList()));
    }
}
